...
Nginx is the **single public entry point** for AGRIBOT. It terminates TLS and forwards
...
## 5. Verify
```bash
curl -k https://38.242.246.126/health           # -k accepts the self-signed cert
# -> {"status":"ok","service":"agribot-api",...}
```
Open `https://38.242.246.126/docs` in a browser (accept the certificate warning) — the
Swagger UI now loads over HTTPS through the proxy.
